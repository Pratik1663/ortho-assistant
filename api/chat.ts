...
function buildWorkflowBlock(
  ...
  if (action === 'consultation') {
    ...
    lines.push(
      ...
      'Two sections are the ones most often skipped, so give them particular attention. Posting and skives covers forefoot posting and heel lifts as well as the rearfoot post — finishing the rearfoot question is not finishing the section. Additions is thirteen items and is easy to pass over entirely; read the presentation against them and say what, if anything, is indicated.',
      'Do not list all thirteen additions one by one. Name the ones the presentation actually supports, and dispose of the rest together in a line. A patient with no forefoot complaint does not need to be walked through met pads, met bars, neuroma pads and met accommodations individually.',
