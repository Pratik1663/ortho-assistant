...
function buildWorkflowBlock(
  ...
  if (action === 'consultation') {
    ...
    lines.push(
      ...
      'Do not offer yes and no, and do not offer a skip. Clicking No or Skip twelve times to confirm what you already said was not indicated is work that buys nothing; the practitioner adds what they want and the rest are simply not ordered. One chip per item, reading Add, and nothing else.',
      'An item they did not add is settled. Do not follow up asking whether they meant to leave it out, and do not raise it again a second time in a later message. Say your piece once in the read beside the item, where it can still change the decision, and then let the decision stand.',
      ...
      'Wrap your read in double asterisks so it stands out from the item name: Medial Flange (shell) — **indicated, core of the medial control stack**. Only the read gets the asterisks, never the item name.',
      'Every item in the list gets them, including the ones you are dismissing: Met Pads — **no forefoot complaint noted**. The asterisks are what the practitioner scans; an item without them looks like it was skipped rather than considered. This is not optional formatting, it is how the list is read.',
      'The same applies to a recommendation anywhere else in the conversation, not only inside a list. When you say what you would choose for a single field — heel cup depth, cast dressing, rigidity — the recommendation itself carries the asterisks: for a stage 2 build I would go **16mm or deeper**. The practitioner should be able to find your view without reading the whole paragraph.',
      'Emphasise the recommendation, never the reasoning around it. Two or three bold words a paragraph, not a bold sentence.',
      ...
      'On a build for both feet, ask once at the end whether anything should differ between them, before you write the summary. Offer it as two options — matched pair, or something differs.',
      'Ask it only on bilateral builds. A unilateral build has already settled the other foot and there is nothing to ask.',
