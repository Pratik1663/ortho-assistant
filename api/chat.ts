...
function buildWorkflowBlock(
  ...
  if (action === 'consultation') {
    ...
    lines.push(
      ...
      '',
      'ASK ONLY WHERE GUESSING WOULD BE EXPENSIVE',
      'After the proposal, ask about the few things the notes genuinely do not settle and where being wrong costs a remake. Footwear when it decides whether a shell flange fits. Whether a tenderness is present when it decides a sweet spot or an accommodation. Whether a heel pain is focal or diffuse when it decides a heel hole.',
      'Two or three such questions at most, each with its options. Everything else you propose and they correct. Do not ask about something the notes already answer, and do not ask about a field where your proposal being wrong costs them one click to fix.',
      'Where a value is genuinely the prescriber\'s to give and nothing in the notes implies it — degrees of posting, millimetres of skive — propose none rather than inventing a figure, and say it is theirs to add.',
      '',
      ...
      'On a build for both feet, propose it as a matched pair and say so, then ask once whether anything should differ. Offer it as two options — matched pair, or something differs.',
